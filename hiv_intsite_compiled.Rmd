...
for (u in uniq_exp) {
  df_plot_all <- df_compiled %>%
    ...
    arrange(desc(location), desc(cell), desc(patient_anon)) %>%
  ...
  heatmap_theme <- theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = 7),
  ...
  p <- egg::set_panel_size(g, height=unit(length(col_of_interest) / 1.5, "cm"),
                          width=unit(length(unique(of_interest$Var1)) / 1.5, "cm") )
...
ggsave(paste0("figs/", "overlap_hotspot_final.png"),
       device = "png", width = 60, height = 40, units = "cm")
```
...
make_clonal_expansion_graph <- function(df, p, exp_type) {
  ...
  # TODO arrange most dominant on bottom
  # TODO label top two dominant in graph (geom label)
  g <- ggplot(df_rel, aes(x = x_axis, y = rel_abund, fill = gene_plot_2)) +
    geom_bar(stat = "identity") +
    geom_text(aes(label = plot_lbl), size = 1, position = position_stack(vjust = 0.5)) +
    theme_classic() +
...
cowplot::save_plot("figs/rel_abund_chronic.svg",
                   chronic_rel_abund, base_width = NULL, base_height = 25, device = "svg")
