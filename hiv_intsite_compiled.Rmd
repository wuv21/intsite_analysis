...
compare_enrichment <- function(all_stats, exp_a, exp_b, save=TRUE) {
  ...
  #       matrix(
  #         c(!!a_sites_lbl, !!b_sites_lbl, total_acute_sites, total_pat_sites),
  #         ncol = 2))$p.value) %>%
  #     ungroup() %>%
...
```{r}
# cd4 trm from mus spleen - can mark 69+
# https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE128197

# tfh from human ln
# https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE130793

tfh_rnaseq <- read.table("rnaseq/GSE130793_LVRNAseq_CD4_gene_counts.txt", header = TRUE)
tfh_rnaseq <- tfh_rnaseq %>%
  dplyr::select(id, geneSymbol, A_1, B_1, C_1)

library(DESeq2)
tfh_rnaseq_matrix <- as.matrix(tfh_rnaseq[, c(3:ncol(tfh_rnaseq))])
rownames(tfh_rnaseq_matrix) <- tfh_rnaseq$id

tfh_col_data <- data.frame(name = colnames(tfh_rnaseq)[c(3:5)])
rownames(tfh_col_data) <- tfh_col_data$name

tfh_dds <- DESeqDataSetFromMatrix(countData = tfh_rnaseq_matrix,
                                  colData = tfh_col_data,
                                  design = ~ 1)

tfh_dds <- DESeq(tfh_dds)
tfh_counts <- as.data.frame(counts(tfh_dds, normalized = T))
tfh_counts$id <- rownames(tfh_counts)

tfh_counts <- tfh_counts %>%
  rowwise() %>%
  dplyr::mutate(avg_exp = mean(c(A_1, B_1, C_1), na.omit = TRUE)) %>%
  left_join(tfh_rnaseq[, c("id", "geneSymbol")], by = "id", keep = FALSE) %>%
  ungroup() %>%
...
get_genes_by_cell <- function(df, .filter_crit) {
  ...
  genes <- toString((df %>%
    ...
    dplyr::filter(!!.filter_crit) %>%
    dplyr::select(in_gene))$in_gene)

  genes <- gsub(" ", "", genes)
  genes <- unlist(str_split(genes, pattern = ","))
  
  overlapping <- data.frame(geneSymbol = genes,
                            stringsAsFactors = FALSE)
  
  return(overlapping)
}

plot_tx_bins <- function(df, title) {
  df %>%
    left_join(tfh_counts[, c("id", "geneSymbol", "bin")], by = "geneSymbol") %>%
    group_by(bin) %>%
    summarise(freq = n() / nrow(.)) %>%
    ggplot(aes(x = as.factor(bin), y = freq)) +
    geom_bar(stat = "identity", fill = "#000000", width = 0.8) +
    labs(title = title,
         x = "Tfh gene expression bins (decreasing -> increasing)",
         y = "Frequency") +
    theme_classic() +
    theme(plot.title = element_text(hjust = 0.5))
}

plot_tx_bins(get_genes_by_cell(df_compiled, cell == "GC-Tfh" | cell == "HLA-DR- GC-Tfh"), "GC-Tfh")
plot_tx_bins(get_genes_by_cell(df_compiled, cell == "CD69+" | cell == "HLA-DR- CD69+"), "CD69+")
plot_tx_bins(get_genes_by_cell(df_compiled, cell == "CD69-" | cell == "HLA-DR- CD69-"), "CD69-")
plot_tx_bins(get_genes_by_cell(df_compiled, cell == "Tcm" | cell == "CD69- Tcm"), "Tcm")
plot_tx_bins(get_genes_by_cell(df_compiled, cell == "Tem" | cell == "CD69- Tem"), "Tem")
```
