...
# same as above but with clone information
df_compiled %>%
  ...
  group_by(exp, patient_anon_v2, location) %>%
  mutate(location_sum = sum(total_abund),
         location_clone_sum = sum(total_clone)) %>%
  ungroup() %>%
  group_by(exp, patient_anon_v2) %>%
  mutate(patient_sum = sum(total_abund),
         patient_clones = sum(total_clone)) %>%
  arrange(exp, desc(patient_sum), desc(location_sum)) %>%
  write.table("df_compiled_grouped_stats_with_clones.csv", col.names = TRUE,
              row.names = FALSE, quote = FALSE, sep = "\t")
```
...
total_chronic_sites <- length(unique(paste(chronic_df$patient_anon_v2, art_df$posID)))

total_pat_sites <- length(unique(paste(df_compiled$patient, df_compiled$posID)))

compare_enrichment <- function(all_stats, exp_a, exp_b) {
  ...
  res <- all_stats %>%
    ...
    ungroup() %>%
    mutate(a_freq = !!a_sites_lbl / (gene_width * total_a_sites),
           b_freq = !!b_sites_lbl / (gene_width * total_b_sites),
           pct_chg = 100 * (b_freq - a_freq) / a_freq) %>%
...
#          chronic_freq = chronic_n_sites / (gene_width * total_chronic_sites),
#          acute_art_pct_chg = 100 * (art_freq - acute_freq) / acute_freq,
#          acute_chronic_pct_chg = 100 * (chronic_freq - acute_freq) / acute_freq,
#          chronic_art_pct_chg = 100 * (art_freq - chronic_freq) / chronic_freq) %>%
