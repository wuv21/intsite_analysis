...
```{r}
count_total_abundance <- function(df) {
  df %>%
...
count_loc_abundance <- function(df) {
  df %>%
    ungroup() %>%
...
count_ind_abundance <- function(df) {
  df %>%
    ungroup() %>%
...
df_compiled %>%
  count_total_abundance() %>%
  count_loc_abundance() %>%
  count_ind_abundance() %>%
  ungroup() %>%
  mutate(exp = factor(exp, levels = c("ART", "Acute", "Chronic"))) %>%
  arrange(exp) %>%
  write.table("df_compiled_grouped_stats_with_clones.csv",
