...
df_compiled <- df_compiled %>%
  dplyr::filter(!is.na(patient_anon)) %>%
...
df_compiled$cell <- sapply(seq_along(df_compiled$cell), function(i) {
  row <- df_compiled[i, ]
  cell <- row$cell
  loc <- row$location
  exp <- str_match(row$run, pattern = "(\\w+)_")[, 1]
...
cowplot::save_plot("figs/rel_abund_art.pdf", art_rel_abund, base_width = NULL, base_height = 25, device = "pdf")

acute_rel_abund <- cowplot::plot_grid(clonal_graphs[["A_D"]], clonal_graphs[["A_E"]], clonal_graphs[["A_F"]], ncol = 1, align = "h")
cowplot::save_plot("figs/rel_abund_acute.pdf", acute_rel_abund, base_width = NULL, base_height = 25, device = "pdf")
...
# circos plot
```{r}
# TODO FIX GRAPH...SOMETHING WRONG WITH THE OVERLAP LINES...
uniq_patients <- unique(df_compiled$patient_anon)
