...
tfh_genes <- toString((df_compiled %>%
  dplyr::filter(cell == "HLA-DR- GC-Tfh" | cell == "GC-Tfh") %>%
  dplyr::filter(in_gene != "FALSE") %>%
  dplyr::select(in_gene))$in_gene)

tfh_genes <- gsub(" ", "", tfh_genes)
tfh_genes <- unlist(str_split(tfh_genes, pattern = ","))

gse21379 <- read.csv("~/Downloads/Gene Set- GSE21379_TFH_VS_NON_TFH_CD4_TCELL_UP.txt",
                     skip = 2,
                     header = FALSE)
gse21379 <- as.character(gse21379$V1)


base::intersect(tfh_genes, gse21379)
```
