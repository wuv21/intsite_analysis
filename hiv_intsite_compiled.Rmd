...
# change order of infection stage
df_compiled <- df_compiled %>%
...
df_compiled %>%
  ...
  write.table("Table_summary.tsv",
              col.names = TRUE, row.names = FALSE, quote = FALSE, sep = "\t")

df_compiled %>%
  dplyr::select(patient_anon_v2, location, cell, exp, posID, abund, gene_plot) %>%
  dplyr::rename(Individual = patient_anon_v2,
         Compartment = location,
         Cell = cell,
         Stage = exp,
         `Sonic_Abundance` = abund,
         `Gene/nearest gene` = gene_plot) %>%
  write.table("Supplemental_File_1.tsv", quote = FALSE, sep = "\t", row.names = FALSE)
```


```{r}
df_clones <- df_compiled %>%
  mutate(clone = ifelse(exp == "Acute", abund >= 3, abund >= 2)) %>%
  group_by(exp, patient_anon_v2, location, cell) %>%
  summarise(total_unique_sites = n(),
            total_abund = sum(abund),
            total_clone = sum(clone)) %>%
  ungroup() %>%
  group_by(exp, cell) %>%
  summarize(all_uniq_sites = sum(total_unique_sites),
            all_total_clone = sum(total_clone))

resident_clones <- df_compiled %>%
  mutate(clone = ifelse(exp == "Acute", abund >= 3, abund >= 2)) %>%
  group_by(exp, patient_anon_v2, location, cell) %>%
  summarise(total_unique_sites = n(),
            total_abund = sum(abund),
            total_clone = sum(clone)) %>%
  mutate(resident = grepl("(GC-Tfh|CD69\\+)", cell)) %>%
  mutate(resident_detailed = ifelse(resident,
                           "Resident\n(CD69+ or GC-Tfh phenotypes)",
                           "Non-resident")) %>%
  ungroup() %>%
  group_by(exp, patient_anon_v2, location, resident_detailed) %>%
  summarise(all_uniq_sites = sum(total_unique_sites),
            all_total_clone = sum(total_clone)) %>%
  ungroup() %>%
  dplyr::mutate(percent = all_total_clone / all_uniq_sites * 100,
                exp_resident = paste(exp, resident_detailed, sep = "_"))

resident_graph <- ggplot(resident_clones, aes(x = exp, y = percent, group = resident_detailed)) +
  geom_bar(aes(fill = resident_detailed),
           width = 0.8, position = position_dodge(width = 0.85), stat = "summary") +
  geom_point(aes(shape = location),
             size = 2,
             position = position_jitterdodge(jitter.width = 0.1, dodge.width = 0.85)) +
  labs(x = "Infection stage",
       y = "% clonal of total unique sites",
       fill = "Cell types",
       shape = "Compartment") +
  scale_fill_manual(values = c("red", "blue")) +
  theme_classic()

ggsave("figs/resident_clonal_expansion.svg", plot = resident_graph,
       device = "svg", width = 6, height = 3)

resident_aov <- aov(percent ~ resident_detailed * exp, data = resident_clones)
summary(resident_aov)
TukeyHSD(resident_aov, which = "exp")


exp_combos <- combn(levels(resident_clones$exp), 2)
exp_p_vals <- apply(exp_combos, MARGIN = 2, function(x) {
  df <- resident_clones %>%
    dplyr::filter(exp %in% x)
  
  print(x)
  wc <- wilcox.test(percent ~ exp, data = df)
  print(wc$p.value)
  
  return(wc)
})

exps <- levels(resident_clones$exp)
residence_p_vals <- sapply(exps, function(x) {
  df <- resident_clones %>%
    dplyr::filter(exp == x)
  
  print(x)
  wc <- wilcox.test(percent ~ resident_detailed, data = df)
  print(wc$p.value)
  
  return(wc)
})

other_combos <- combn(unique(resident_clones$exp_resident), 2)
other_p_vals <- apply(other_combos, MARGIN = 2, function(x) {
  df <- resident_clones %>%
    dplyr::filter(exp_resident %in% x)
  
  wc <- wilcox.test(percent ~ exp_resident, data = df)
  
  return(wc$p.value)
})

View(data.frame(t(other_combos)) %>%
  mutate(p_val = other_p_vals,
...
# save clone table
df_clones %>%
  dplyr::mutate(percent = format(round(all_total_clone / all_uniq_sites * 100, 2), nsmall = 2)) %>%
  dplyr::rename(Stage = exp,
                Subset = cell,
                `Total unique sites` = all_uniq_sites,
                `Clonal sites` = all_total_clone,
                `% clonal` = percent) %>%
  write.table("clone_ratio.tsv", col.names = TRUE, row.names = FALSE, quote = FALSE, sep = "\t")
...
calc_overlap <- function(ids, bin_index) {
  # get unique ones that are duplicated...
  abund_ids <- unique(ids[duplicated(ids)])
  total_count <- 0
...
uniq_patients <- sort(unique(df_compiled$patient_anon_v2))
graphs <- list()
...
full_plot <- cowplot::plot_grid(plotlist = graphs, nrow = 3)
ggsave(filename = "figs/global_overlap_ecdf_all.svg", plot = full_plot, device = "svg")
ggsave(filename = "figs/global_overlap_ecdf_all.png", plot = full_plot,
       dpi = "retina", width = 8, height = 6, device = "png")
```
...
for (u in uniq_exp) {
  ...
  saveRDS(p_comb, glue("{fig_rds_prefix}/overlap_hotspot_{u}.rds"))
  saveRDS(p, glue("{fig_rds_prefix}/overlap_hotspot_{u}_p1.rds"))
  saveRDS(p2, glue("{fig_rds_prefix}/overlap_hotspot_{u}_p2.rds"))
...
make_clonal_expansion_graph <- function(df, p, exp_type) {
  ...
  g <- ggplot(df_rel, aes(x = x_axis, y = rel_abund, fill = gene_plot_2)) +
    ...
    geom_text(aes(label = plot_lbl), size = 8/.pt, position = position_stack(vjust = 0.5)) +
    theme_classic() +
...
save_rel_abund_graphs <- function(prefix, stage) {
  g_rel_abund <- cowplot::plot_grid(clonal_graphs[[glue("{prefix}1")]],
                                    clonal_graphs[[glue("{prefix}2")]],
                                    clonal_graphs[[glue("{prefix}3")]],
                                    ncol = 1, align = "h")
  
  cowplot::save_plot(glue("figs/rel_abund_{stage}.svg"),
                     g_rel_abund,
                     base_width = NULL, base_height = 25, device = "svg")
  
  return("saved graph")
}

save_rel_abund_graphs("T", "art")
save_rel_abund_graphs("A", "acute")
save_rel_abund_graphs("C", "chronic")

# saveRDS(art_rel_abund, glue("{fig_rds_prefix}/art_rel_abund.rds"))
# saveRDS(acute_rel_abund, glue("{fig_rds_prefix}/acute_rel_abund.rds"))
# saveRDS(chronic_rel_abund, glue("{fig_rds_prefix}/chronic_rel_abund.rds"))
```
...
df_anno <- data.frame(gr_anno) %>%
  mutate(feature = case_when(grepl("Intron", annotation) ~ "Intron",
    ...
    TRUE ~ "Other")) %>%
  mutate(feature = factor(feature, levels = rev(c("Intron", "Distal intergenic", "Exon", "Other")))) %>%
  ...
  mutate(x_axis = paste0(gsub(" ", "\n", cell),
                         ...
                         group_abund,
                         ")"),
         y_pos = cumsum(percent_feature) - 0.5 * percent_feature) %>%
  ungroup() %>%
  mutate(exp = factor(exp, levels = c("Acute", "Chronic", "ART")),
...
# site_annot_graph <- ggplot(df_anno, aes(x = 1.5, y = percent_feature)) +
#   geom_bar(aes(fill = feature), stat = "identity", width = 0.5) +
#   labs(fill = "Feature",
#        y = "% sonic abundance",
...
#   xlim(0.5, 2.5) +
#   geom_text(aes(y = y_pos, x = 2.25, label = round(percent_feature, digits = 1)), size = 8/.pt) +
#   geom_text(data = df_anno %>%
#               group_by(exp, location, cell) %>%
#               summarise(group_abund = group_abund) %>%
#               unique(), y = 0, x = 0.5, aes(label = group_abund), size = 8/.pt) +
#   scale_fill_viridis_d() +
#   facet_wrap(~ exp * location * cell,
#              labeller = facet_label_clean)

site_annot_graph <- ggplot(df_anno, aes(x = x_axis, y = percent_feature)) +
...
ggsave(filename = "figs/site_annot_subset_v3.svg", plot = site_annot_graph, device = "svg", width = 17, height = 2)
```
...
plot_tx_bins <- function(df, bins, title) {
  df_graph <- df %>%
    ...
    mutate(bin = ifelse(is.na(bin), "NA", bin),
           bin = factor(as.character(bin), levels = c("NA", as.character(1:10)))) %>%
    group_by(bin) 
...
tfh_tx_plots <- lapply(seq_along(filter_crit), function(i) {
  plot <- tx_wrapper(df_compiled, filter_crit[i], tfh_bins, titles[i])
  return(plot)
})

nontfh_tx_plots <- lapply(seq_along(filter_crit), function(i) {
  plot <- tx_wrapper(df_compiled, filter_crit[i], nontfh_bins, titles[i])
  return(plot)
})
...
# revision, stage comparisons
```{r}
# set ids
df_anno <- data.frame(gr_anno) %>%
  mutate(annotation = str_remove(annotation, " \\(ENST.*\\)"),
         annotation = ifelse(grepl("Intron$", annotation), "Intron", annotation),
         annotation = ifelse(grepl("Exon$", annotation), "Exon", annotation))

df_compiled_anno <- data.frame(gr_long_anno) %>%
  mutate(annotation = str_remove(annotation, " \\(ENST.*\\)"),
         annotation = ifelse(grepl("Intron$", annotation), "Intron", annotation),
         annotation = ifelse(grepl("Exon$", annotation), "Exon", annotation))

# look at distribution on chromosome
chr_lengths <- read.csv("chr_lengths.csv")
chr_lengths <- setNames(as.numeric(chr_lengths$length), chr_lengths$chr)

df_compiled_anno %>%
  mutate(chrom = str_match(string = seqnames, pattern = "(?<=chr).*"),
         chr_rel = start / chr_lengths[chrom]) %>%
  ggplot(aes(x = chr_rel, after_stat(density), color = exp)) +
  geom_freqpoly(binwidth = 0.01) +
  theme_classic() +
  labs(color = "Infection stage",
       x = "Normalized chromosomal position",
       y = "Density") +
  ggsave(filename = "figs/rel_chr_loc.svg", device = "svg", width = 6, height = 2)
...
df_anno %>%
  ...
  ungroup() %>%
  mutate(total_prop = total_abund / sum(total_abund) * 100)
...
# by infection stage
df_anno %>%
  group_by(patient_anon_v2, exp, annotation) %>%
  summarize(annotation_count = sum(abund)) %>%
  ungroup() %>%
  group_by(patient_anon_v2) %>%
  mutate(annotation_prop = annotation_count / sum(annotation_count) * 100) %>%
  # ggpubr::compare_means(formula = annotation_prop ~ exp, data = ., group.by = "annotation")
  ggplot(aes(x = annotation, y = annotation_prop, group = exp)) +
  geom_bar(aes(fill = exp), stat = "summary", fun = "mean", position = position_dodge2(preserve = "single")) +
  geom_point(position = position_dodge(width = 0.9)) +
  lg_panel_style +
  labs(x = "Annotation",
       y = "Proportion of sites (%)",
       fill = "Infection stage") +
  ggsave(filename = "figs/gene_annot_all_exp.svg", device = "svg", width = 8.5, height = 4)


# by blood vs tissue
df_anno %>%
  mutate(location2 = ifelse(location == "PBMC", "Blood", "Tissue")) %>%
  group_by(patient_anon_v2, exp, location2, annotation) %>%
  summarize(annotation_count = sum(abund)) %>%
  ungroup() %>%
  group_by(patient_anon_v2, exp, location2) %>%
  mutate(annotation_prop = annotation_count / sum(annotation_count) * 100) %>%
  ggplot(aes(x = annotation, y = annotation_prop, group = location2)) +
  geom_bar(aes(fill = location2), stat = "summary", fun = "mean",
  ...
  labs(x = "Annotation",
       y = "Proportion of sites (%)",
       fill = "Compartment") +
  ggsave(filename = "figs/gene_annot_all_compartment.svg", device = "svg", width = 8.5, height = 4)
...
df_anno %>%
  ...
  mutate(resident_detailed = ifelse(resident,
                           "Resident\n(CD69+ or GC-Tfh phenotypes)",
                           "Non-resident")) %>%
  group_by(patient_anon_v2, exp, resident_detailed, annotation) %>%
  summarize(annotation_count = sum(abund)) %>%
  ungroup() %>%
  group_by(patient_anon_v2, exp, resident_detailed) %>%
  mutate(annotation_prop = annotation_count / sum(annotation_count) * 100) %>%
  ggplot(aes(x = annotation, y = annotation_prop, group = resident_detailed)) +
  geom_bar(aes(fill = resident_detailed), stat = "summary", fun = "mean",
           position = position_dodge2(preserve = "single")) +
  geom_point(position = position_dodge(width = 0.9)) +
  facet_wrap(~ exp) +
  lg_panel_style +
  labs(x = "Annotation",
       y = "Proportion of sites (%)",
       fill = "Cell phenotype") +
  ggsave(filename = "figs/gene_annot_all_res.svg", device = "svg", width = 8.5, height = 4)


plot_by_infection_stage <- function(df, .grouping_var) {
  .grouping_var <- enquo(.grouping_var)
  
  g_list <- by(data = df, INDICES = df$exp, FUN = function(sm_df) {
      sm_df <- droplevels(sm_df)
      g <- sm_df %>%
        ggplot(aes(x = annotation, y = annotation_prop, group = !!.grouping_var)) +
        geom_bar(aes(fill = !!.grouping_var), stat = "summary", fun = "mean",
                 position = position_dodge2(preserve = "single")) +
        geom_point(aes(color = cell), position = position_dodge(width = 0.9)) +
        theme_classic() +
        ...
        scale_fill_manual(values = c("#000000", "#cccccc"), guide = "none") +
...
df_anno %>%
  mutate(location2 = ifelse(location == "PBMC", "Blood", "Tissue"),
         annotation = ifelse(!annotation %in% c("Intron", "Distal Intergenic"), "Other", annotation),
         annotation = factor(annotation, levels = c("Intron", "Distal Intergenic", "Other"))) %>%
  group_by(patient_anon_v2, exp, location2, cell, annotation) %>%
  summarize(annotation_count = sum(abund)) %>%
  ungroup() %>%
  group_by(patient_anon_v2, location2, cell) %>%
  mutate(annotation_prop = annotation_count / sum(annotation_count) * 100)
  # ggpubr::compare_means(formula = annotation_prop ~ location2, data = ., group.by = c("exp", "annotation")) %>%
  # View()
  plot_by_infection_stage(., location2) %>%
  patchwork::wrap_plots() %>%
  ggsave(filename = "figs/gene_annot_loc.svg", device = "svg", width = 7, height = 4)
...
df_anno %>%
  ...
  mutate(resident_detailed = ifelse(resident,
         ...
         annotation = ifelse(!annotation %in% c("Intron", "Distal Intergenic"), "Other", annotation),
         annotation = factor(annotation, levels = c("Intron", "Distal Intergenic", "Other"))) %>%
  group_by(patient_anon_v2, exp, resident_detailed, cell, annotation) %>%
  summarize(annotation_count = sum(abund)) %>%
  ungroup() %>%
  group_by(patient_anon_v2, resident_detailed, cell) %>%
  mutate(annotation_prop = annotation_count / sum(annotation_count) * 100) %>%
  ggpubr::compare_means(formula = annotation_prop ~ resident_detailed, data = ., group.by = c("exp", "annotation")) %>%
  View()
  plot_by_infection_stage(., resident_detailed) %>%
  patchwork::wrap_plots() %>%
  ggsave(filename = "figs/gene_annot_res.svg", device = "svg", width = 7, height = 4)
```
