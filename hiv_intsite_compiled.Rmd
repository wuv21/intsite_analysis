...
max_abund <- max(df_compiled$abund)
# abund_scale <- colorRampPalette(brewer.pal(9, "Greys")[c(7:9)])(100)

# TODO separate legend by acute and art treated...since number of uniq cells do differ

uniq_patients <- c("A", "B", "C")
for (p in uniq_patients) {
  df_chord_test <- df_compiled %>%
    ...
    mutate(posID = factor(posID, levels = unique(posID)),
           geneplot_posID = paste(nearest_gene, posID),
           ...
           cell_track_color = ifelse(group_n == 1,
                                     ...
                                     "#FFFFFFFF"),
           abund_color = ifelse(abund >= 2, "#000000", "#CCCCCC"))
...
grid.newpage()
grid.draw(leg)
ggsave("figs/chord_legend.svg", device = "svg", width = 8)
```
