...
gc_vs_non <- vella_rnaseq %>%
  dplyr::select(id, geneSymbol, A_1, B_1, C_1, A2, B2, C2)

gc_vs_non_matrix <- as.matrix(gc_vs_non[, c(3:ncol(gc_vs_non))])
rownames(gc_vs_non_matrix) <- gc_vs_non$id

col_data <- data.frame(name = colnames(gc_vs_non)[c(3:ncol(gc_vs_non))],
                       cells = c(rep("GCTfh", 3), rep("nonTfh", 3))) %>%
  mutate(cells = factor(cells, levels = c("nonTfh", "GCTfh")))
rownames(col_data) <- col_data$name

dds <- DESeqDataSetFromMatrix(countData = gc_vs_non_matrix,
                              colData = col_data,
                              design = ~ cells)
...
up_tfh <- res %>%
  ...
  dplyr::rename(id = row) %>%
  left_join(gc_vs_non, by = "id", keep = FALSE)

length(intersect(get_genes_by_cell(df_compiled, cell == "GC-Tfh" | cell == "HLA-DR- GC-Tfh")$geneSymbol,
          up_tfh$geneSymbol))

length(intersect(get_genes_by_cell(df_compiled, cell == "CD69+" | cell == "HLA-DR- CD69+")$geneSymbol,
          up_tfh$geneSymbol))

length(intersect(get_genes_by_cell(df_compiled, cell == "CD69-" | cell == "HLA-DR- CD69-")$geneSymbol,
          up_tfh$geneSymbol))

length(intersect(get_genes_by_cell(df_compiled, cell == "Tcm" | cell == "CD69- Tcm")$geneSymbol,
          up_tfh$geneSymbol))

length(intersect(get_genes_by_cell(df_compiled, cell == "Tem" | cell == "CD69- Tem")$geneSymbol,
          up_tfh$geneSymbol))
...
filter_crit <- quos(c(cell == "GC-Tfh" | cell == "HLA-DR- GC-Tfh",
