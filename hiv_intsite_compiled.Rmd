...
make_heatmaps <- function(df, annot_ref, filter_type) {
  ...
  if (!file.exists(epi_roc_fn)) {
    ...
    sites_mrcs_epigenetic <- annotate_epigenetic(sites_mrcs, annot_ref)
  ...
  epi <- heatmap_to_ggplot2(epi_roc_fn, "epigenetic", histoneorder_for_heatmap, filter_type)
  
  return(grid.arrange(gen, epi, nrow = 1))
}
...
# df_sites_meta <- convert_to_mrc_acceptable(df_pre_mrc_custom)
# 
# 
# 
# sites_mrcs <- generate_mrcs(df_sites_meta)
